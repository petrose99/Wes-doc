...
Company currency (#457): pane row "Company currency" = code · "locked since ‹event›" (`currencyLockEvent`, `lib/admin/companies.ts`, day + en-US short month — en-GB gives "Sept") · Owner-only inline `Change` → `components/admin/change-currency-dialog.tsx` (`ConfirmDialog`, renders `role="alertdialog"`; locked / push-in-flight / failure lines `role="alert"`), shared by the Integrations card's Switch. Creation forms use `components/workspace/company-country-currency-fields.tsx` (LS: LSL/ZAR select; ZA: text + hidden input). Seed `scripts/dev/seed-457.ts`; round `scripts/wayfinder-autopilot/round-457.mjs`.
