...
| Automatic Logoff | §164.312(a)(2)(iii) | ⚠️ requires deployment config | Mechanism enforced in code (`lib/supabase/middleware.ts`), sized by `SESSION_IDLE_TIMEOUT_MINUTES` — but the default changed from 15 (the value this row was originally verified against) to 30 days as of 2026-09-08, once it became clear the 15-minute value was applied globally, to every workspace, not only ones needing it. `SESSION_IDLE_TIMEOUT_MINUTES` is one process-wide value; it cannot vary per workspace. **A deployment running any workspace with `hipaaMode` on must set it back to 15 (or another value ≤ 30) in that deployment's env**, or this control is not actually 15-minute automatic logoff regardless of what the code default is — see `lib/config.ts`'s comment on the variable. No workspace on the reference deployment has `hipaaMode` enabled as of this note, so 30 days is what is actually live there. An idle session is signed out server-side (refresh token revoked via `signOut()`), not just cookie-cleared client-side; that part is unaffected by the default. Left as the Free-plan app-level fallback rather than Supabase Auth's own Inactivity Timeout (Pro plan and above) for the reason originally noted — if the project is later upgraded to Pro, the two controls stack redundantly, which is harmless. |
