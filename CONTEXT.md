...
**Payout destination**:
Where a Payee's payment goes: a Lesotho bank account (bank, branch code, account number) at Standard Lesotho Bank, FNB Lesotho or Nedbank Lesotho, or a mobile-money wallet (network, number, account name) on one of Chaperone's five networks (C-Pay, M-Pesa, EcoCash, MyWallet, Khetsi). A Payee has one active destination, of either kind — never several and never a default among several. Bank and wallet numbers are validated only in form (digits, length, the shape a network or bank expects); DocuBite verifies neither kind against the bank or network. A bank destination becomes payable only once its route has credentials — Chaperone carrying it is not assumed.
_Avoid_: Bank details (now the bank-kind destination's fields only), account details (ambiguous with the Payer account)

**Payout details change**:
An edit to a Payee's Payout destination after it was first entered: a changed bank, branch code or account number; a changed wallet network, number or account name; or a switch between bank and wallet (a new account type or holder name on the same destination, or the same number reformatted, is not a change). It holds that Payee — no new batch, and any pending or approved batch holding them is rejected — until an Owner acknowledges or rejects it. Acknowledging records who and when and asks for no proof: confirming the destination with the supplier is the Owner's responsibility, not DocuBite's. Rejecting restores the last acknowledged destination. Several edits before a decision are one change, from the last acknowledged destination to the latest; editing back to the acknowledged destination ends it. Anyone who edited the change cannot decide it unless no other Owner is left to (then it reads self-acknowledged). A held Payee reads *Payout details changed* wherever it can't be paid. A Payee's first Payout destination is not a change, whichever kind it is — but once a destination exists, replacing it with the other kind (a Payee's first bank account, added after they'd only ever had a wallet) is a change like any other, not a second "first": one destination per Payee means there is nothing to add beside it, only to replace. Payout destinations are only ever saved by a person: details read off an invoice or a Bank confirmation are offered to pre-fill, never written, and an invoice whose destination differs from the one on file fails a Check — a bank invoice against a wallet on file (or the reverse) differs on kind alone, before any field is compared.
_Avoid_: Bank details change (superseded — a destination is not always a bank), Verified account, bank verification, AVS (DocuBite does not verify accounts); "verified" names only supplier trust

**Bank confirmation**:
The supplier's (or Claimant's) own letter confirming their Payout destination, optionally attached to a version of those details. It pre-fills them and is shown to the Owner deciding a Payout details change; it never marks the details verified and is never required.
