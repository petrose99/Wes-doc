 ...
 *
 * React 19's resource-preloading APIs (not raw <link> JSX — that produced a second, differently-
 * ordered preload tag alongside Next's own hoisted one for the same URL; harmless since browsers
 * coalesce identical preloads, but these are the framework-supported way to say the same thing and
 * they dedupe correctly by construction) get DNS/TCP/TLS negotiated and the script bytes fetched in
 * parallel with the page's own load, so by the time the effect creates its own <script> tag
 * pointing at the same URL, the browser typically already has it.
 *
 ...
 * it does now. preconnect is unaffected either way — it opens a connection rather than fetching a
 * resource, which CSP's fetch directives don't govern.
 *
 * Gated on the same server flag GoogleButton itself gates on — a deployment with no Google client
 * id should not warm a connection, or fetch a script, it is never going to load.
 *
 * Called during render, not at module scope: these APIs inject into the response THIS render is
 * producing, so they have to run on every request, not once when the module is first loaded into
 * the server process. */
