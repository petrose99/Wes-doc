 ...
 * plain list with Open, so the address is never a dead end. */
export default async function CompaniesPage({ params }: { params: Promise<{ workspaceId: string }> }) {
  ...
  return <AdminPage title="Companies" intro="Each company is its own workspace with its own people, documents and ledger. You are configuring the one named in the left nav.">
