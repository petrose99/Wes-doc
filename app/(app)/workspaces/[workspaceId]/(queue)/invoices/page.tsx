...
export async function InvoicesQueuePage({ params, searchParams, selectedDocumentId = null }: {
  ...
  // #258: the five processing-state keys plus the ledger's posted/paid. A stale "unreviewed"/
  // "reviewed" value from an old URL just doesn't match — the chip shows nothing selected.
  const statusFilter = status === "cancelled" || status === "needs_attention" || status === "in_review" || status === "touchless" || status === "approved" || status === "posted" || status === "paid" ? status : undefined
  ...
  // #281: the bulk Post button's target connection. Null (off, none, or inactive) still renders
  // the button (§3 never hides it); the dialog reads every row as ineligible and the
  // connection-failure band (a later build step) says why.
  const connectionId = await getActiveIntegrationConnectionId(workspaceId)
  // #281 spec.md §6: the connection-failure band — the same connection the Post button targets.
  const connectionBandStatus = await getLedgerConnectionBandStatus(workspaceId)
