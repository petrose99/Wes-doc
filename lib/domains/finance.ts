...
export const FINANCE_TEMPLATES = [
  {
    ...
    fields: [
      ...
      { key: "supplier_address", label: "Supplier address", type: "string", instruction: "Supplier's postal address exactly as printed. Leave blank if not shown — do not infer.", required: false },
      ...
      { key: "has_tax_invoice_wording", label: "Tax invoice wording shown", type: "boolean", instruction: "True if the document displays a heading or label such as \"Tax Invoice\", \"VAT Invoice\", or plain \"Invoice\" (jurisdictions that require this wording for input-tax recovery, e.g. ZA VAT Act s20(4)(a), LS VAT Act s.24(8)). False for a plain receipt, quote, or statement with no such wording.", required: false },
      { key: "recipient_name", label: "Recipient name", type: "string", instruction: "Name of the customer or recipient being billed, exactly as printed (the \"bill to\"/\"sold to\" party). Leave blank if not shown.", required: false, negative: "not the supplier, not the payment recipient bank" },
      { key: "recipient_address", label: "Recipient address", type: "string", instruction: "Recipient's postal address exactly as printed. Leave blank if not shown.", required: false },
      { key: "recipient_vat_number", label: "Recipient VAT number", type: "string", instruction: "Recipient's VAT/tax registration number, if shown on the invoice. Leave blank if not shown — do not infer.", required: false },
  ...
  },
  {
    ...
    fields: [
      ...
      { key: "merchant_address", label: "Merchant address", type: "string", instruction: "Merchant's postal address exactly as printed. Leave blank if not shown — do not infer.", required: false },
      ...
      { key: "currency_code", label: "Currency shown", type: "string", instruction: "Literal ISO 4217 code printed on the document; do not convert", required: false },
      { key: "supplier_vat_number", label: "Merchant VAT number", type: "string", instruction: "Merchant's VAT/tax registration number exactly as printed. Leave blank if not shown — do not infer.", required: false },
      { key: "has_tax_invoice_wording", label: "Tax invoice wording shown", type: "boolean", instruction: "True if the document displays a heading or label such as \"Tax Invoice\", \"VAT Invoice\", or plain \"Invoice\"/\"Receipt\" (jurisdictions that require this wording for input-tax recovery, e.g. ZA VAT Act s20(4)(a), LS VAT Act s.24(8)). False if no such wording appears.", required: false },
      { key: "recipient_name", label: "Recipient name", type: "string", instruction: "Name of the customer or recipient being billed, exactly as printed. Leave blank if not shown — most receipts don't name one.", required: false },
      { key: "recipient_address", label: "Recipient address", type: "string", instruction: "Recipient's postal address exactly as printed. Leave blank if not shown.", required: false },
      { key: "recipient_vat_number", label: "Recipient VAT number", type: "string", instruction: "Recipient's VAT/tax registration number, if shown. Leave blank if not shown — do not infer.", required: false },
      { key: "subtotal", label: "Subtotal", type: "number", instruction: "Amount before tax, if shown separately from the total", required: false, mergeStrategy: "last", retrievalHints: ["subtotal", "net total", "amount before tax", "sub total"], negative: "not the grand total, not any single line amount" },
