...
const envSchema = z.object({
  ...
  // Page-level retrieval gating for extraction (OCBC pipeline, Stage 1). On by default: for
  // documents longer than DOCUMENT_RETRIEVAL_MIN_PAGES the pipeline scores pages per required
  // field (hybrid BM25 + optional dense fusion), sends only the top-K union to the LLM, and
  // sweeps any remaining pages ONLY if a required field is still missing. Kill switch when off.
  DOCUMENT_PAGE_RETRIEVAL: z.enum(["true", "false"]).default("true"),
  DOCUMENT_RETRIEVAL_MIN_PAGES: z.coerce.number().int().positive().default(12),
  DOCUMENT_RETRIEVAL_TOP_K: z.coerce.number().int().positive().default(3),
...
const config = {
  ...
  documents: {
    ...
    pageRetrievalEnabled: env.DOCUMENT_PAGE_RETRIEVAL === "true",
