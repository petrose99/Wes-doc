...
export function Switch({ checked, onCheckedChange, label, describedBy, disabled = false, busy = false, id }: {
  checked: boolean
  onCheckedChange: (next: boolean) => void
  label: string
  describedBy?: string
  disabled?: boolean
  /** A save in flight: the control stays focusable (disabling it would drop focus to body) and
   * ignores further clicks until it settles. */
  busy?: boolean
  id?: string
}) {
  return <button type="button" id={id} role="switch" aria-checked={checked} aria-label={label} aria-describedby={describedBy}
    aria-busy={busy || undefined} disabled={disabled} onClick={() => { if (!busy) onCheckedChange(!checked) }}
