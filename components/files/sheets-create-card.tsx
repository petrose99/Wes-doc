...
export function SheetsCreateCard({ icon, title, description, workspaceId, href, badge }: {
  ...
  return (
    <button
      onClick={handleClick}
      disabled={creating}
      className="group flex items-center gap-3 rounded-xl border border-[#e6ebf1] bg-white px-4 py-3 text-left shadow-panel transition-[border-color,box-shadow,transform] duration-150 hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-md disabled:opacity-60 sm:flex-col sm:items-start sm:gap-0 sm:p-5"
    >
