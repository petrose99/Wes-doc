    ...
    {loading || !sharing
      ...
      : <div className="divide-y">
          <section className="space-y-3 px-5 py-4">
            ...
            <ul className="space-y-1">
              <li className="flex items-center gap-2 rounded px-1 py-1.5 text-sm">
                <span className="min-w-0 flex-1 truncate text-slate-700">Everyone in this workspace</span>
                <span className="shrink-0 text-xs font-medium text-slate-400">Owner</span>
              </li>
              {sharing.people.map((person) => <li key={person.email} className="flex items-center gap-2 rounded px-1 py-1 text-sm">
                <span className="min-w-0 flex-1 truncate text-slate-700" title={person.email}>{person.email}</span>
          ...
          <section className="space-y-2 px-5 py-4">
            <h3 className="text-sm font-semibold text-slate-900">General access</h3>
            {sharing.hipaaMode
              ...
              : <>
                  <div className="flex items-center gap-2">
                    <span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${sharing.linkAccess === "none" ? "bg-slate-100 text-slate-500" : "bg-emerald-100 text-emerald-700"}`}>
                      {sharing.linkAccess === "none" ? <Lock className="h-4 w-4" /> : <Globe className="h-4 w-4" />}
                    </span>
                    <select aria-label="General access" className="rounded-md border border-slate-300 px-2 py-1.5 text-sm" value={sharing.linkAccess} onChange={(event) => void changeLinkAccess(event.target.value)}>
                  ...
                  {sharing.linkAccess !== "none" && <div className="flex items-center gap-2 pt-1">
                    <input readOnly aria-label="Share link" className={`${inputClass} bg-slate-50 text-slate-500`} value={sharing.shareUrl} onFocus={(event) => event.currentTarget.select()} />
                    <button type="button" className="inline-flex shrink-0 items-center gap-1.5 rounded-md border px-2.5 py-1.5 text-sm font-medium text-slate-700 hover:bg-slate-50" onClick={() => void copyLink()}>
