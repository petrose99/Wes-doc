...
export function FinanceProposalPart({ workspaceId, state, output }: {
  ...
  return <div className="space-y-1.5 rounded-lg border border-indigo-200 bg-indigo-50 px-3 py-2.5">
    <p className="text-sm text-indigo-900">{output.summary}</p>
    <div className="flex gap-2">
      ...
      <button type="button" disabled={pending} onClick={() => setResolved("dismissed")}
        className="rounded-md border border-slate-300 px-2.5 py-1 text-xs font-semibold text-slate-600 hover:bg-slate-100 disabled:opacity-50">
        Dismiss
      </button>
    </div>
  </div>
