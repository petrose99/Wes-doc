...
export function ReviewInbox({ workspaceId, tasks, currentStatus, members, financeAgentEnabled, documentSearchEnabled }: {
  ...
  const assistantButton = financeAgentEnabled && <button
    ...
    className={`ml-auto flex shrink-0 items-center gap-1.5 rounded-md border px-2.5 py-1 text-xs font-medium transition-colors ${assistantOpen ? "border-emerald-300 bg-emerald-50 text-emerald-800" : "border-slate-200 text-slate-600 hover:bg-slate-50"}`}>
  ...
  if (!tasks.length) return <div className="space-y-3">
    ...
    <div className="flex gap-4">
      <div className="flex-1 rounded-md border border-dashed p-8 text-center text-sm text-slate-500">
      ...
      {assistantOpen && <AssistantPanel
        ...
        className="flex w-80 shrink-0 flex-col rounded-md border bg-slate-50"
