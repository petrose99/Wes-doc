...
export function Pipeline() {
  ...
  return (
    <section
      id="pipeline"
      ref={ref}
      className="relative border-y border-cream-200 bg-white py-14 md:py-22"
    ...
    >
      <div className="relative mx-auto max-w-6xl px-5">
        ...
        <div className="mt-9 grid grid-cols-[repeat(auto-fit,minmax(250px,1fr))] gap-4">
          <div className="rounded-2xl border border-cream-200 border-t-[3px] border-t-stone-300 bg-white p-4 shadow-panel">
            <div className="flex items-center justify-between gap-2.5">
              <span className="text-[0.84rem] font-bold text-stone-900">Inbox</span>
              <span className="rounded-full bg-slate-100 px-2.5 py-0.5 text-[0.74rem] font-bold text-slate-600">{counts.inbox}</span>
            </div>
            <p className="mb-3 mt-1.5 text-[0.76rem] text-stone-500">Uploaded, being read</p>
            <div className="flex flex-col gap-1.5">
              {["statement-mar.pdf", "IMG_3312.jpg"].map((name, i) => (
                <div key={name} className={`flex items-center gap-2 rounded-lg border border-slate-100 bg-slate-50 px-2.5 py-2 text-[0.76rem] text-slate-700 ${played ? IN : ""}`} style={played ? { animationDelay: `${0.1 + i * 0.2}s` } : undefined}>
          ...
          <div className="rounded-2xl border border-cream-200 border-t-[3px] border-t-indigo-500 bg-white p-4 shadow-panel">
            <div className="flex items-center justify-between gap-2.5">
              <span className="text-[0.84rem] font-bold text-stone-900">To review</span>
              <span className="rounded-full bg-indigo-600 px-2.5 py-0.5 text-[0.74rem] font-bold text-white">{counts.review}</span>
            </div>
            <p className="mb-3 mt-1.5 text-[0.76rem] text-stone-500">Waiting on a person</p>
            <div className="flex flex-col gap-1.5">
              <div className={`flex flex-wrap items-center gap-2 rounded-lg border border-violet-100 bg-violet-50 px-2.5 py-2 text-[0.76rem] text-indigo-800 ${played ? IN : ""}`} style={played ? { animationDelay: ".75s" } : undefined}>
                <span className="min-w-0 flex-1 truncate">receipt-cafe.heic</span>
                <span className="rounded bg-red-100 px-1.5 py-px text-[0.64rem] font-bold text-red-800">total</span>
              </div>
              <div className={`flex flex-wrap items-center gap-2 rounded-lg border border-violet-100 bg-violet-50 px-2.5 py-2 text-[0.76rem] text-indigo-800 ${played ? IN : ""}`} style={played ? { animationDelay: ".95s" } : undefined}>
                <span className="min-w-0 flex-1 truncate">INV-4471 (1).pdf</span>
                <span className="rounded bg-indigo-100 px-1.5 py-px text-[0.64rem] font-bold text-indigo-800">dupe</span>
              </div>
            </div>
          </div>

          <div className="rounded-2xl border border-cream-200 border-t-[3px] border-t-emerald-500 bg-white p-4 shadow-panel">
            <div className="flex items-center justify-between gap-2.5">
              <span className="text-[0.84rem] font-bold text-stone-900">Ready</span>
              <span className="rounded-full bg-emerald-700 px-2.5 py-0.5 text-[0.74rem] font-bold text-white">{counts.ready}</span>
            </div>
