  ...
  return (
    <section className="relative overflow-hidden bg-cream-50">
      ...
      <div className="relative mx-auto grid max-w-6xl items-center gap-14 px-5 py-16 lg:grid-cols-[0.92fr_1.08fr] lg:py-24">
        <div>
          {copy.eyebrowBadge && <p className="mb-3.5 text-sm font-semibold uppercase tracking-[.16em] text-emerald-700">{copy.eyebrowBadge}</p>}
          ...
          <div className="mt-8 flex flex-wrap items-center gap-x-6 gap-y-3">
            <Link href={copy.ctaHref} className="group inline-flex h-12 items-center justify-center gap-2 rounded-full bg-emerald-950 px-7 text-base font-semibold text-white shadow-sm transition-colors hover:bg-emerald-900">
              {copy.ctaLabel}<ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-0.5" />
            </Link>
            <a href={copy.anchorHref} className="text-sm font-semibold text-stone-700 underline decoration-stone-300 underline-offset-4 transition-colors hover:text-stone-950">
              {copy.anchorLabel}
            </a>
          </div>
          <p className="mt-3 text-sm text-stone-500">{copy.ctaNote}</p>
        </div>
