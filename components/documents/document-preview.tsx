...
export function DocumentPreview({ src, filename, mimeType, className = "" }: {
  ...
  if (mimeType.startsWith("image/")) {
    return <div className={`overflow-auto bg-slate-100 p-3 ${className}`}>
  ...
  return <div className={`flex min-h-0 flex-col ${className}`}>
    ...
    <div className="flex items-center gap-2 border-t bg-slate-50 px-3 py-1.5 text-xs text-slate-500">
      <span className="min-w-0 flex-1 truncate">Preview not showing?</span>
      <a className="inline-flex shrink-0 items-center gap-1 font-medium text-slate-700 hover:text-slate-900 hover:underline" href={src} target="_blank" rel="noreferrer"><ExternalLink className="h-3 w-3" />Open in new tab</a>
      <a className="inline-flex shrink-0 items-center gap-1 font-medium text-slate-700 hover:text-slate-900 hover:underline" href={src} download={filename}><Download className="h-3 w-3" />Download</a>
    </div>
  </div>
}
