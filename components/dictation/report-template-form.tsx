  ...
  return (
    <div className="space-y-5 text-sm">
      <section>
            ...
            return (
              <li key={index} className={`flex flex-wrap items-center gap-2 rounded border px-3 py-2 ${isNew ? "border-dashed border-emerald-300 bg-emerald-50/30" : ""}`}>
                ...
                <span className="flex shrink-0 gap-0.5">
                  <button type="button" aria-label={`Move ${slot.label} up`} disabled={index === 0} onClick={() => setSlots((state) => move(state, index, index - 1))} className="rounded p-1 text-muted-foreground hover:bg-slate-100 disabled:opacity-30"><ArrowUp className="h-3.5 w-3.5" /></button>
                  <button type="button" aria-label={`Move ${slot.label} down`} disabled={index === slots.length - 1} onClick={() => setSlots((state) => move(state, index, index + 1))} className="rounded p-1 text-muted-foreground hover:bg-slate-100 disabled:opacity-30"><ArrowDown className="h-3.5 w-3.5" /></button>
        ...
        <button type="button" onClick={addSlot} className="mt-2 flex items-center gap-1 rounded-md border border-dashed border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">
      ...
      <section>
              ...
              <li key={index} className={`space-y-1.5 rounded border px-3 py-2 ${isNew ? "border-dashed border-emerald-300 bg-emerald-50/30" : ""}`}>
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    className="min-w-0 flex-1 rounded-md border border-input px-2 py-1 text-sm focus:border-emerald-400 focus:outline-none" />
                  <span className="flex shrink-0 gap-0.5">
                    <button type="button" aria-label={`Move ${section.title} up`} disabled={index === 0} onClick={() => setSections((state) => move(state, index, index - 1))} className="rounded p-1 text-muted-foreground hover:bg-slate-100 disabled:opacity-30"><ArrowUp className="h-3.5 w-3.5" /></button>
                    <button type="button" aria-label={`Move ${section.title} down`} disabled={index === sections.length - 1} onClick={() => setSections((state) => move(state, index, index + 1))} className="rounded p-1 text-muted-foreground hover:bg-slate-100 disabled:opacity-30"><ArrowDown className="h-3.5 w-3.5" /></button>
        ...
        <button type="button" onClick={addSection} className="mt-2 flex items-center gap-1 rounded-md border border-dashed border-slate-300 px-2.5 py-1.5 text-xs font-medium text-slate-600 hover:bg-slate-50">
