...
- **The hand-off carries a condensed build plan.** Ordered steps, each with the spec lines it comes from, what exists and what is left. A continuation resumes from the plan, reading spec sections and pre-flight parts by line range for the steps it is about to build, one area primer, and reused components by the symbols it calls (grep, then a bounded range) — never whole files end to end. A session that re-reads everything spends its budget before its first edit (#286 lost two build sessions this way).
