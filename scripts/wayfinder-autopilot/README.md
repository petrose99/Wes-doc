...
Capture rounds: `capture-round.mjs` is the shared runner for the measure and close phases (fresh context per state, detector injection, residue filter, keyboard probes, per-state error isolation, `detector.json` + `keyboard.json`); a ticket's round script is a state list on top of it. The token-guard hook refuses `git commit` and any Write/Edit of the hand-off while it is over `WAYFINDER_HANDOFF_MAX_LINES` (80 in this repo's `config.sh`); context-guard nags once as well, and the run-log row records the length. The same hook refuses `gh issue close` on a ticket that touched `app/` or `components/` until the report's `scores:` line carries close-critique ≥ 30 and close-evaluate ≥ 80. In the build phase it also refuses the measure session's readers — reads of the evaluate/include digests and impeccable's critique/audit references, and any Agent whose brief is scoring (critique, evaluate, heuristics, re-score): the gate's evidence is `gate.mjs` + detector counts, the scores are the measure phase's, once (#380's G2 ran six scoring agents, 2026-09-22). A `Stop` hook (`hooks/stop-guard.sh`) refuses the end of an autopilot turn — at most `WAYFINDER_STOP_BLOCKS_MAX` (2) times — while the tree has uncommitted work, neither the hand-off nor the report was written this session, or the driver's dev/live servers are still up, naming what is owed; "this session" is measured against the ctx file, which the driver now recreates (with its flag files) per session. Past the hard context stop it stays quiet and the driver's WIP path takes over. `hooks/flail-guard.sh` is the circuit breaker for one session (PostToolUseFailure, PostToolUse, PreToolUse on Bash): it counts failures by signature — the error lines with numbers, hashes and tmp paths blanked, so one root cause behind varied commands counts as one — and at `WAYFINDER_FLAIL_MAX` (3) tells the session to stop retrying and check the precondition, at twice that to hand off with the blocker; the exact command that tripped is refused until some other call succeeds. `hooks.test.ts` fires every hook at the call it exists to refuse (and the ordinary call beside it) — a hook that crashes exits 1, which Claude Code treats as allow, so a guard is only trusted while its drill passes.
